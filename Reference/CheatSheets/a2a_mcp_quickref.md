...
| **Primary ADK type IN** | `RemoteA2aAgent(agent_card_url=...)` — consume an external A2A agent as if it were a local sub-agent. | `MCPToolset(connection_params=...)` — consume external MCP server's tools as if they were local `FunctionTool`s. |
...
| **Discovery artifact** | `AgentCard` — JSON manifest describing the agent's capabilities, schemas, auth. Served at `/.well-known/agent.json` (or similar). | Tools/resources/prompts list — fetched at handshake time from the MCP server. |
...
| **Auth surface** | A2A auth schemes in the AgentCard (OAuth, API key). | MCP auth via the transport's auth (HTTP headers, OAuth, etc.). |
...
# AgentCard served at http://localhost:8000/.well-known/agent.json
```
...
booking_agent = RemoteA2aAgent(
    agent_card_url="https://hotels.example.com/.well-known/agent.json",
)
...
from google.adk.tools.mcp.session_manager import StdioServerParameters
